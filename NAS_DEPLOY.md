...
네이버 로그인이 필요한 경우 프로젝트 폴더에 `.env` 파일을 만듭니다.
...
크롤링 확인이 필요할 때는 웹 화면에서 `브라우저 화면 보기`를 켜고 실행합니다. Docker 컨테이너는 `Xvfb` 가상 디스플레이에서 headed Edge를 실행하고, 웹앱이 최신 브라우저 화면을 이미지로 보여줍니다.

Docker 이미지에는 Linux용 Microsoft Edge Stable이 설치됩니다. `docker-compose.yml`의 `REVIEW_BROWSER_MODE=edge` 설정으로 NAS에서도 Edge 기반으로 수집합니다.
